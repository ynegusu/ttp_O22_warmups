...
SELECT title, rental_rate,
	CASE
	WHEN rental_rate = (SELECT rental_rate FROM lowest_rate) THEN 0.10 
	WHEN rental_rate = (SELECT rental_rate FROM rate_next_above_1) THEN 1 
	END AS new_rate
FROM film
WHERE rating = 'PG-13';
...
FROM film as f
	...
	JOIN actor as a ON a.actor_id=fa.actor_id
WHERE f.film_id IN (SELECT film_id FROM films_list) AND
 fa.actor_id not in (SELECT actor_id FROM top_actor); 
